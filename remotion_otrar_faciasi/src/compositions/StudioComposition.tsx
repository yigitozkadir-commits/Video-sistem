...
const SceneLayer: React.FC<{
  ...
  // Otrar-local fix (2026-08-09): the template's plain even split
  // (durationFrames / visuals.length) is what produced the historical
  // Başkurtlar bug - a 6s video clip assigned an even slot of the full
  // scene duration looped 5.6-13.7x and only a human watching the
  // finished render caught it (CLAUDE.md section 10). Here a visual may
  // carry an explicit `durationFrames` (set by
  // scripts/generate_otrar_scenes_data.py to the video's real ffprobe
  // duration for video slots); visuals without one split the REMAINING
  // frames evenly, so a video slot never has to loop past ~1 cycle and
  // the pre-render checklist's loop-ratio check has something honest to
  // verify against.
  const explicitTotal = visuals.reduce((sum, v) => sum + (v.durationFrames ?? 0), 0);
  const autoVisuals = visuals.filter((v) => v.durationFrames === undefined);
  const autoSlotDuration = autoVisuals.length > 0
    ? Math.floor(Math.max(0, durationFrames - explicitTotal) / autoVisuals.length)
    : 0;

  let cursor = 0;
  const slots = visuals.map((visual, i) => {
    const isLastVisual = i === visuals.length - 1;
    const ownDuration = visual.durationFrames ?? autoSlotDuration;
    const thisSlotDuration = isLastVisual ? durationFrames - cursor : ownDuration;
    const slotStartFrame = cursor;
    cursor += thisSlotDuration;
    return { visual, slotStartFrame, thisSlotDuration, isLastVisual };
  });
  ...
  return (
    <>
      ...
      {slots.map(({ visual, slotStartFrame, thisSlotDuration, isLastVisual }, i) => {
        return (
